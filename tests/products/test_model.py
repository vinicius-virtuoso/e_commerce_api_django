...
class TestProduct(TestCase):
  ...
  def setUpTestData(cls) -> None:
    ...
    cls.category.product.add(cls.product_mock)

    cls.image_product = ImageProduct.objects.create(image_url='https://res.cloudinary.com/dnkw0zu2x/image/upload/v1688328201/django_commerce/no-photo.png', product=cls.product_mock)
  ...
  def test_product_description_max_length(self):
    description_max_length = self.product_mock._meta.get_field('description').max_length
  ...
  def test_image_product_max_length(self):
    image_max_length = self.image_product._meta.get_field('image_url').max_length

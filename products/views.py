...
class ProductCreateListView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        products_exists = Product.objects.filter(
            slug__iexact=data_request.get("slug", None)
        ).exists()
        if products_exists:
            return Response(
                {"detail": "Product already exists."}, status.HTTP_409_CONFLICT
            )

        categories = [
            value for key, value in data_request.items() if key.startswith("category_")
        ]
